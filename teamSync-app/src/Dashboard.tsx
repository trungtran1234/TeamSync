...
const Dashboard: React.FC = () => {
  ...
  const [flaggedMeetings, setFlaggedMeetings] = useState<Set<number>>(
    new Set(),
  );
        ...
        <div className="flex space-x-6 mb-8 border-b border-slate-700">
          {["recent", "flagged", "all"].map((tab) => (
            <button
              ...
              className={`pb-4 px-2 text-sm font-medium capitalize ${activeTab === tab ? "text-violet-400 border-b-2 border-violet-400" : "text-slate-400 hover:text-slate-200"}`}
            >
              {tab} Meetings
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {filteredMeetings.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="bg-slate-800 rounded-xl p-6 shadow-md hover:shadow-lg border border-slate-700 relative"
              >
                ...
                >
                  <Flag
                    className={`h-5 w-5 ${isFlagged ? "text-violet-400" : "text-slate-400"}`}
                  />
                </button>
              </div>
            );
